...
- 仪表盘：状态 / 设置（端口、工作区、API Key、版本、开机自启、**自定义背景**）/ 更新（一键升级 + 历史版本回滚）/ 日志 / **插件管理器**。
- **插件管理器**：浏览/搜索官方插件仓库（GitHub topic `dsh-plugin`，显示 **star 数**与**简介**，支持按 star / 最近更新排序），一键安装（git 安装）/卸载，内置 pnpm 离线可用；**每次安装/卸载前自动备份，可一键回退**。
